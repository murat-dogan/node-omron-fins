...
_processMemoryAreaRead = function (buf, rinfo) {
    ...
    return { remotehost: rinfo.address, sid: sid, command: command, response: response, values /** values:data */ };
};
...
FinsClient.init = function (port, host, options) {
    ...
    function receive(buf, rinfo) {
        ...
        if (readCbList[msg.sid])
            readCbList[msg.sid](msg.response, msg.values);
        delete readCbList[msg.sid];
        readCbList[msg.sid] = null;
    }
...
FinsClient.prototype.read = function (address, regsToRead, callback, responseCb) {
    var self = this;
    ...
    readCbList[self.header.SID] = responseCb;
